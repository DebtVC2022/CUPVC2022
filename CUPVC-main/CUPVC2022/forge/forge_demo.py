...
def forge_to_wavefrom(bad_path, seq_len, pad_len):
    ...
    hidden_good, _ = DebtVC.reconstruct(mel_spe_good.to(device), timestamp_good, dec_inp_good.view((1, seq_len, 80)), timestamp_good)
    ...
    hidden, _ = DebtVC.reconstruct(mel_spe.to(device), timestamp, dec_inp.view((1, seq_len, 80)), timestamp)
    ...
    new_expand_hidden = DebtVC.reconstruct.expand(new_hidden)
    
    d_in = DebtVC.reconstruct.dec_embedding(dec_inp.view(1, seq_len, 80), timestamp)
    d_out = DebtVC.reconstruct.decoder(d_in, new_expand_hidden)
    d_out = DebtVC.reconstruct.projection(d_out)
    return d_out
